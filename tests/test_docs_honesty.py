...
def test_mock_keeper_is_localnet_only_source() -> None:
    ...
    assert "def check(self, app: Application)" in src
    assert 'arc4_signature("check()uint64")' in src
...
def test_pages_draws_escrow_graphs_from_history() -> None:
    assert "drawEscrowBars" in APP_JS
    ...
    assert "escrow-canvas" in html
    assert "history-canvas" in html
    assert "timeline-canvas" in html
    assert "sql.js/1.11.0" in html


def test_append_history_never_writes_deploy() -> None:
    append = (ROOT / "scripts" / "append_history.mjs").read_text()
    assert "Never writes docs/deploy.json" in append
    assert "deploy.json untouched" in append or "does not modify" in append
    assert "writeFileSync(deployPath" not in append
    assert "history.json" in append


def test_due_json_unsigned_keeper_probe() -> None:
    """Unsigned TestNet keeper probe; never invents a deadman app/upkeep."""
    ...
    assert int(due.get("deadmanAppId") or 0) == 0
    assert int(due.get("deadmanUpkeepId") or 0) == 0
    ...
    # LocalNet proof ids must not leak into due.json as a TestNet deadman
    assert int(due.get("deadmanAppId") or 0) != int(LOCALNET.get("appId") or 0)
