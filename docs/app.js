...
async function loadDueSnapshot() {
  try {
    ...
    // deadman as deployed. deadmanAppId must stay 0 here.
    if (Number(due.deadmanAppId || 0) !== 0) return null;
    if (Number(due.deadmanUpkeepId || 0) !== 0) return null;
    return due;
  } catch (_) {
    return null;
  }
...
async function main() {
  ...
  if (appId <= 0) {
    ...
    // reports a live TestNet keeper round — deadman itself is undeployed.
    setNetworkMeta("LocalNet proof only · not on TestNet · unaudited · MBR-safe");
    let sub = "not deployed · keeper " + keeper + " · LocalNet app 1118";
  ...
  try {
    ...
    const keeperOnChain = readGlobal(g, "keeper_app");
