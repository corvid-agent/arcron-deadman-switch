#!/usr/bin/env python3
"""LocalNet listen: set_keeper + configure + poke + mock inner-call check.
...
SET_KEEPER = Method.from_signature("set_keeper(uint64,pay)void")
CONFIGURE = Method.from_signature("configure(uint64)void")
POKE = Method.from_signature("poke()uint64")
CLAIM = Method.from_signature("claim()uint64")
MK_CHECK = Method.from_signature("check(uint64)void")

# Silence window in rounds. After poke, advance past last_poke + TIMEOUT.
TIMEOUT_ROUNDS = 1
# Escrow above MBR so claim has surplus to pull after trip.
ESCROW_MICRO = 100_000
MBR_MICRO = 100_000
...
def bump_rounds(algod: AlgodClient, addr: str, pk: bytes, n: int) -> list[str]:
    """DevMode advances last-round on each confirmed txn. No mnemonic printed."""
    txids: list[str] = []
    for _ in range(max(0, n)):
        pay = PaymentTxn(
            sender=addr, sp=params(algod, fee=1000), receiver=addr, amt=0
        )
        txid = algod.send_transaction(pay.sign(pk))
        wait_confirmed(algod, txid)
        txids.append(txid)
    return txids
...
def main() -> None:
    ...
    before = decode_global(live)
    print(f"target Deadman appId={app_id} creator={creator}", file=sys.stderr)
    if int(before.get("keeper_app") or 0) != 0:
        sys.exit(
            f"refusing: Deadman appId={app_id} already has keeper_app="
            f"{before.get('keeper_app')} — recreate first"
        )
    if int(before.get("timeout_rounds") or 0) != 0:
        sys.exit(
            f"refusing: Deadman appId={app_id} already configured "
            f"timeout_rounds={before.get('timeout_rounds')} — recreate first"
        )
    ...
    if creator and addr != creator:
        sys.exit(f"refusing: signer {addr} is not Deadman creator {creator}")
    ...
    dead_addr = get_application_address(app_id)
    calls: list[dict] = []

    # Fund app MBR + escrow in the set_keeper payment (contract requires >= MBR).
    fund_amt = MBR_MICRO + ESCROW_MICRO
    pay = PaymentTxn(
        sender=addr, sp=params(algod, fee=1000), receiver=dead_addr, amt=fund_amt
    )
    rec = call_abi(
        algod,
        addr,
        ...
        SET_KEEPER,
        method_args=[mock_id],
        foreign_apps=[mock_id],
        extra_payment=pay,
        fee=2000,
    )
    rec["keeperAppId"] = mock_id
    rec["paymentMicroAlgos"] = fund_amt
    calls.append(rec)
    print(f"set_keeper txid={rec['txid']} round={rec['round']}", file=sys.stderr)

    rec = call_abi(
        algod, addr, signer, app_id, CONFIGURE, method_args=[TIMEOUT_ROUNDS]
    )
    rec["timeoutRounds"] = TIMEOUT_ROUNDS
    calls.append(rec)
    print(f"configure txid={rec['txid']} round={rec['round']}", file=sys.stderr)

    rec = call_abi(algod, addr, signer, app_id, POKE)
    rec["hook"] = "poke"
    rec["via"] = "creator_check_in"
    calls.append(rec)
    poke_round = int(rec["round"] or 0)
    print(f"poke txid={rec['txid']} round={poke_round}", file=sys.stderr)

    # Need Global.round > last_poke + timeout. DevMode: each confirmed tx bumps round.
    # After poke at R with timeout=1, need round >= R+2 → bump at least twice.
    bump_txids = bump_rounds(algod, addr, pk, TIMEOUT_ROUNDS + 1)
    print(f"bumped {len(bump_txids)} rounds after poke", file=sys.stderr)
    ...
    rec["hook"] = "check"
    rec["via"] = "mock_keeper.check"
    calls.append(rec)
    print(
        f"check via mock txid={rec['txid']} round={rec['round']} inners={rec['innerCount']}",
        file=sys.stderr,
    )

    rec = call_abi(algod, addr, signer, app_id, CLAIM, fee=3000)
    rec["hook"] = "claim"
    rec["via"] = "permissionless_pull"
    calls.append(rec)
    ...
    payload = {
        ...
        "global": {
            ...
            "timeout_rounds": after.get("timeout_rounds", 0),
            "last_poke_round": after.get("last_poke_round", 0),
            "tripped": after.get("tripped", 0),
        },
        "notes": (
            ...
            "Did not poke upkeep 81 or 87. check() trips a flag only; "
            "claim() pulls surplus above 100_000 MBR."
        ),
    }
        ...
        json.dumps(
            {
                ...
                "appId": app_id,
                "mockKeeperAppId": mock_id,
                "tripped": payload["global"]["tripped"],
                "last_poke_round": payload["global"]["last_poke_round"],
