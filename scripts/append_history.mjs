...
if (String(listen.genesisId || "").toLowerCase().includes("mainnet")) {
  die("refusing: genesis looks like MainNet");
}

const g = listen.global || {};
let escrow = 0;
for (const call of listen.calls || []) {
  if (call && call.method === "set_keeper" && call.paymentMicroAlgos != null) {
    escrow = Number(call.paymentMicroAlgos);
    break;
  }
}
const sample = {
  ...
  escrow,
  last_poke_round: Number(g.last_poke_round || 0),
  tripped: Number(g.tripped || 0),
  timeout_rounds: Number(g.timeout_rounds || 0),
