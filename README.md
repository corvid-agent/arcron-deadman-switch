...
This pass (2026-09-07 ~2:46 PM MT): recreate Deadman **appId 1134** on `dockernet-v1` (confirmed round 105). `python scripts/localnet_listen.py` created mock keeper **1135**, wired `set_keeper` + `configure(1)` + `poke`, inner-called `check` (1 inner → tripped), then `claim` pulled surplus above MBR. Global after listen: keeper_app=1135, timeout_rounds=1, last_poke_round=110, tripped=1. LocalNet last-round after listen: 114. Did not spend the TestNet bank. Did not poke upkeep 81 or 87.
