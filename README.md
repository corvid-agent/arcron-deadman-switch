...
This pass (2026-09-10 ~6:47 PM MT): recreate Deadman **appId 1040** on `dockernet-v1` (confirmed round 32). `python scripts/localnet_listen.py` created mock keeper **1041**, wired `set_keeper` + `configure(1)` + `poke`, inner-called `check` (1 inner → tripped), then `claim` pulled surplus above MBR. Global after listen: keeper_app=1041, timeout_rounds=1, last_poke_round=37, tripped=1. LocalNet last-round after listen: 41. Did not spend the TestNet bank. Did not poke upkeep 81 or 87.
